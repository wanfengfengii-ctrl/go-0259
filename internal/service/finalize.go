...
func (s *Service) Finalize(id domain.TaskID, req FinalizeRequest) (FinalizeResult, error) {
	...
	finalState := stateForConclusion(conclusion)
...
func stateForConclusion(c arbiter.Conclusion) inspection.TaskState {
	switch c {
	case arbiter.ConclusionTransfer:
		return inspection.StateTransferable
	case arbiter.ConclusionIsolate:
		return inspection.StateContaminationIsolated
	case arbiter.ConclusionCancel:
